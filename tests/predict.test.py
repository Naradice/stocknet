...
def training_auto_encoder(data_client, batch_size, observationDays, processes, epoch_num = -1, hidden_layer_num = 5, middle_layer_size = 48, version=1):
    ...
    model_name = f'{kinds}_{frame}min/ohlc/test_AE-{str(hidden_layer_num)}-{str(middle_layer_size).zfill(2)}_v{str(version)}'
    ...
    tr.training_loop(model, optimizer, epoch_num)
...
if __name__ == "__main__":
    ...
    batch_size = 32
    hidden_layer_size = 5
    middle_layer_size = 48
    ####################
    epoc_num = -1
    
    training_auto_encoder(data_client, batch_size, observationDays, processes, epoc_num, hidden_layer_num=hidden_layer_size, middle_layer_size=middle_layer_size)