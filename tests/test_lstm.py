...
class TestLSTMModel(unittest.TestCase):
    def test_training_lstm(self):
        ...
        dataset = ds.ShiftDataset(data_client=data_client, observationLength=observationLength, in_columns=ohlc, out_columns=ohlc, shift=1, isTraining=True)
        train_dl = DataLoader(dataset, batch_size = batch_size, drop_last = True, shuffle=False, pin_memory=True)
        ds_val = ds.ShiftDataset(data_client=data_client, observationLength=observationLength, in_columns=ohlc, out_columns=ohlc, shift=1, isTraining=False)
