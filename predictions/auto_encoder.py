...
def auto_encoder_macd(file, client_columns, client_date_column, frame, target_column="Close", epoc_num=-1, version=1):
    ...
    data_client = fc.CSVClient(
        file=file, frame=frame, columns=client_columns, date_column=client_date_column, idc_processes=[macd_ps], post_process=processes
    )
    columns = []
    ...
    batch_size = 32
    hidden_layer_num = 5
    # should be less than data length * column num
    middle_layer_size = 12

    model_name = f"macd/AE-{str(hidden_layer_num)}-{str(middle_layer_size).zfill(2)}_v{str(version)}"
    dataset = ds.Dataset(
        data_client=data_client, observationLength=observationLength, in_columns=columns, out_columns=columns, merge_columns=True, isTraining=True
    )
    training_lenear_auto_encoder(
        dataset=dataset,
...
def auto_encoder_ohlc(file, client_columns, client_date_column, epoc_num=-1, version=1):
    processes = [fc.utils.MinMaxPreProcess(scale=(-1, 1))]
...
if __name__ == "__main__":
    ...
    date_column = "time"
    version = 1
    # auto_encoder_macd(file_path, client_column, date_column, "close", -1, version)
    auto_encoder_ohlc(file_path, client_column, date_column, 1, version)
