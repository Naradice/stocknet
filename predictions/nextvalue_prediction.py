...
def next_ohlc_individual(epoc=-1, h_layer_sizes=[1, 2, 4, 8], target_column="close"):
    ...
    for h_layer in h_layer_sizes:
        ...
        dataset = ds.ShiftDataset(
            ...
            out_columns=learning_target_columns,
            shift=shift,
        )
...
def next_nv():
    ...
    training_lstm_model(
        dataset=dataset, batch_size=batch_size, hidden_layer_num=hidden_layer_size, version=version, epoc_num=epoc_num, model_name=model_name
    )
...
def next_high_low(epoc=-1, h_layer_sizes=[2, 4, 8, 16], target_columns=["open", "high", "low", "close"]):
    for h_layer in h_layer_sizes:
        ...
        training_lstm_model(
            ...
            loss_fn=loss_fn,
            activation_for_output=activate_function,
        )
