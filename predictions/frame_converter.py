...
def roll_down(epoc=-1, h_layer_sizes = [4,8, 16], target_columns = ["open", "high", "low", "close"]):
    ...
    for h_layer in h_layer_sizes:
        ...
        dataset = ds.MultiFrameDataset(data_client, observationLength=observationDays, pre_processes=processes,in_frame=5,out_frame=30,in_columns=learning_target_columns, out_columns=["close"])
