...
class BCEnv(gym.Env):
    def __init__(
        ...
        # add columns
        self.__ohlc_columns = [
            self.ohlc_columns_dict["Open"],
            self.ohlc_columns_dict["High"],
            self.ohlc_columns_dict["Low"],
            self.ohlc_columns_dict["Close"],
        ]
        if "open" in columns:
        ...
        self.observation_space = gym.spaces.Box(low=-1, high=1, shape=ob_shape)
    ...
    def reset(self):
        ...
        _min, _max = self.data_client.get_min_max(
            column, data_length=self.max_step
        )  # When client is change, this need to be implemente d in the other way
...
class BCStopEnv(BCEnv):
    ...
    def __init__(
        ...
        data_client: Client,
        max_step: int,
        stop_loss_point: int = 0.05,
        stop_profit_point: int = 0.1,
        usebb=False,
        frames: list = None,
        columns=["High", "Low", "Open", "Close"],
        observationDays=1,
