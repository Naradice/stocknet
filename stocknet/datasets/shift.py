...
class ShiftDataset(Dataset):
    ...
    def __init__(self, data_client, observationLength=1000, idc_processes=[], pre_processes=[], in_columns=["Open", "High", "Low", "Close"], out_columns=["Open", "High", "Low", "Close"], shift=1, merge_input_columns=False, seed = None, isTraining=True):
        self.shift = shift
        super().__init__(data_client, observationLength, idc_processes, pre_processes, in_columns=in_columns, out_columns=out_columns, merge_columns=merge_input_columns, seed=seed, isTraining=isTraining)
